...
| 常用快捷键 | 段落 `Ctrl + 0`；加粗 `Ctrl + B`；斜体 `Ctrl + I`；删除线 `Alt + Shift + 5`；下划线 `Ctrl + U`；链接 `Ctrl + K`；行内代码 <code>Ctrl + `</code>；清除样式 <code>Ctrl + \</code> |
...
| 操作项 | 说明 |
| :--- | :--- |
| 保存形式 | fenced code block，例如 <code>```ts</code> 到 <code>```</code> |
...
![Nomo 图标](./src-tauri/icons/icon.png "本地图片示例"){align=center width=160}

图片节点保存为标准 Markdown 图片语法，并可追加 `{align=center width=160}` 这类扩展属性。`align` 支持 `left`、`center`、`right`；`width` 支持像素数字或百分比。
