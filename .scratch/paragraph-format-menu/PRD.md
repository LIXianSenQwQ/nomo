...
当前 Nomo 的“段落”和“格式”菜单还没有形成稳定的信息架构：部分块级结构放在格式菜单里，标题层级直接铺在一级菜单中，菜单能力与 Typedown 式轻量体验、Typora 式语义编辑心智之间还没有清晰取舍。用户希望先不实现全部功能，而是先确定段落菜单和格式菜单的产品规格：常用能力可见，低频密集能力收进二级菜单，决定保留但尚未实现的能力点击后给出短暂提示。
