use crate::models::{RecentDocument, RecentFileInput, SettingInput, SettingRecord, SnapshotInput, SnapshotRecord};
...
#[tauri::command]
pub(crate) fn remember_recent_file(app: AppHandle, input: RecentFileInput) -> Result<(), String> {
    let connection = open_database(&app)?;
    let now = now_ts();
    let modified_at = crate::file_system::file_modified_at(&input.path);
...
#[tauri::command]
pub(crate) fn list_recent_files(app: AppHandle) -> Result<Vec<RecentDocument>, String> {
    query_recent_files_with_limit(&app, 20)
}
...
#[tauri::command]
pub(crate) fn list_document_snapshots(app: AppHandle, path: String) -> Result<Vec<SnapshotRecord>, String> {
    let connection = open_database(&app)?;
...
pub(crate) fn query_recent_files<R: Runtime>(app: &AppHandle<R>) -> Result<Vec<RecentDocument>, String> {
    query_recent_files_with_limit(app, 8)
}

pub(crate) fn get_setting_value<R: Runtime>(app: &AppHandle<R>, key: &str) -> Result<Option<String>, String> {
    let connection = open_database(app)?;
    let mut statement = connection
...
fn query_recent_files_with_limit<R: Runtime>(app: &AppHandle<R>, limit: i64) -> Result<Vec<RecentDocument>, String> {
    let connection = open_database(app)?;
    let mut statement = connection
        .prepare(
